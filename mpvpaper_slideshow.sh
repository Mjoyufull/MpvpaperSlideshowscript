...
# Process cleanup delay in seconds
cleanupDelay=3
...
display_cycle() {
    ...
    # Delayed kill of old instances
    (
        sleep $cleanupDelay  # Let new wallpapers fully render first
        if [ -n "$old_pids" ]; then
    ...
    log_message "Scheduled old instance termination in $cleanupDelay seconds"
}
